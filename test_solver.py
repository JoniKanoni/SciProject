...
_FOLDERNAMES = ['infinite_well', 'finite_well', 'harmonic',
               'double_well_linear', 'double_well_kubic', 'asym_well']


@pytest.mark.parametrize("foldernames", _FOLDERNAMES)
def test_with_reference_potential(foldernames):
    ...
    __, xmin, xmax, xnum, __, __, inttype, pot, __ = reader.get_data(path)
...
def test_with_reference_energies_wavefuncs(foldernames):
    ...
    potential_known = np.loadtxt(path + 'potential.dat')
    mass, __, __, xnum, first_val, last_val, __, __, __ = reader.get_data(path)
    ...
    assert np.all(np.square(wavefuncs_known) - np.square(wavefuncs_test)
                    <= np.square(wavefuncs_known) * 0.1 + 10 ** -15)
