...
def main():
    ...
    mass, xmin, xmax, xnum, first, last, int_type, pot, inputpath = reader.get_data(args.input)

    #interpolate data and create hamiltonian
    potential = ip.potential_grid(xmin, xmax, xnum, pot[:, 0], pot[:, 1], int_type)
    hamiltonian = ip.hamilton_operator(potential, mass, xnum)

    #find energies/eigenvalues and wavefunctions
    energies, wavefuncs = solver.qm_wavefct(hamiltonian, xnum, first,
                                            last, potential[1:xnum - 1, 0])
